...
| `acknowledgement_kind` | string | `ax_confirmed`, `unverified_timeout`, `no_evidence`, or `not_applicable`. |
...
- **`pasted_unverified`** — a paste chord was sent, but macOS could not poll a value to confirm it at all (no focused Accessibility element, or a secure/password field that withholds its value by design). After a fixed grace period the paste is treated as landed and the clipboard is restored per policy, same as `pasted`; `acknowledgement_kind` is `unverified_timeout`.
...
- **`skipped`** — the sanitizer decided the transcript should not be sent or staged at all (for example, nothing printable survived cleanup, or a multi-line transcript hit terminal mode's newline restriction). No clipboard action occurs.
