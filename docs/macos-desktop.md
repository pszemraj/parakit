...
When the whole transcript cannot be found, a newly visible 32-character leading or trailing window still counts. A terminal scrolls the head of a long paste off the top of the screen and a bounded field truncates the tail, but either end appearing as a new occurrence (not merely already present) is real evidence rather than coincidence: a natural-language run of that length is effectively unique against whatever the field held beforehand.

Very short transcripts, under 12 normalized characters, do not use occurrence matching at all: text that short, such as `ok`, can appear inside unrelated growth such as `token` purely by chance. They require either an exact baseline-to-current insertion after whitespace normalization or the exact selection and value-length transition expected from the paste. Between that floor and 32 characters, a transcript is long enough to trust a whole-transcript occurrence-count *increase* over the pre-chord baseline on its own, even without a leading/trailing window: unrelated churn near the inserted text, such as an editor's Slate/React tree re-rendering around a staged attachment or a terminal redrawing a spinner frame, no longer breaks confirmation, as long as the count of occurrences actually grew rather than the transcript merely already being present.
