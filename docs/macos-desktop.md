...
After a paste chord is sent, macOS does not restore the previous clipboard contents after a fixed delay. It polls the focused Accessibility element's `AXValue` for evidence that the target consumed the paste, at 40 ms intervals against a 1800 ms deadline. Each Accessibility request has its own 250 ms messaging timeout so an unresponsive target cannot hold the worker for macOS's longer system default. Insertion resolves to one of three tiers:
