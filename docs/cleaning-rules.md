...
Number conversion uses `text2num` with a threshold of five. Isolated values strictly below five remain words, while five and larger values and recognized multi-token expressions become digits. Multi-point versions are parsed component by component through `text2num`; parakit then joins those validated components with periods. Generic formatting passes compact short uppercase identifiers and split digit groups without maintaining a list of product names.
